...
async function init(){
    ...
    const defaultcoords = '?47,5,7';
    const queryString = window.location.search || defaultcoords;
    let coords;

    if (isNaN(window.location.search.substring(1, 2))){
        coords = defaultcoords.substring(1).split(",");
    } else {
        coords = queryString.substring(1).split(",");
    }

    var map = L.map('map').setView([coords[0], coords[1]], coords[2]);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors | &copy; <a href="https://www.here.com/">HERE</a> | <a href="https://train-empire.com">Train Empire</a> | <a href="?do=contribute">Contribuer</a> | <a href="https://github.com/MaisClement/MyLinesMap">Code Source</a>' 
    }).addTo(map);
...
// Place les points + Repport si non trouvé
    ...
    for (var i = 0; i < stop.stop_points.length; i++){
        ...
        markers.push(L.marker([lat, lon]).addTo(map).bindPopup(name));
    }    

    all = L.layerGroup(markers).addTo(map);
}
